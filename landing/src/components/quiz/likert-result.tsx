  ...
  return (
    <div
      className={cn(
        ...
        isPrimary ? "border-2" : "border-border/50 bg-card"
      )}
      style={isPrimary ? { borderColor: `${dim.themeColor}40`, backgroundColor: `${dim.themeColor}06` } : undefined}
    >
      ...
      <p className="text-base text-muted-foreground leading-relaxed mb-4">{dim.description}</p>
      ...
      <div className="flex items-start gap-2.5 text-sm rounded-xl bg-indigo-50/70 px-4 py-3">
        ...
        <span className="text-indigo-900/80 leading-relaxed">{dim.growthEdge}</span>
      </div>
    </div>
  );
}

export function LikertResult({
  result,
  quizMeta,
  onRetake,
  shared,
  ...
  const primary = result.primaryDimension;
